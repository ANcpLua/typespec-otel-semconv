...
export function indexAttributes(groups) {
  const byId = new Map();
...
export function partitionGroupsByType(groups) {
...
export function bucketByDomain(byId) {
...
function emitMain(domains, sets) {
  ...
  lines.push(``);
  // Side-effect import so the compiler sees the library's $decorators map at
  // compile time — without this, `extern dec otelEntity(...)` in _decorators.tsp
  // can't resolve to its JS impl.
  lines.push(`import "../dist/index.js";`);
  lines.push(`import "./_decorators.tsp";`);
...
export function bucketGroupsByDomain(groups, prefixes) {
  const buckets = new Map();
  for (const g of groups) {
...
// Union of every domain that contributes at least one symbol to the generated
// library. Mirrors the `allDomains` calculation inside main(); exported so the
// linter rule's KNOWN_DOMAINS literal can be cross-checked against the YAML
// source by a Vitest globalSetup, killing the drift between the two.
export function readSemconvDomains() {
...
function emitKnownDomainsLookup(byId) {
  ...
  const lines = [
    ...
    `export const KNOWN_DOMAINS: ReadonlySet<string> = new Set<string>(${JSON.stringify(sorted)});`,
    ``,
  ];
  writeFileSync(resolve(SRC_DIR, "known-domains.ts"), lines.join("\n"), "utf8");
...
function emitEntityIdentifyingLookup(entityGroups) {
  ...
  for (const e of entityGroups) {
    ...
    if (!name) continue;
    const ids = (e.attributes ?? [])
      .filter((a) => a.role === "identifying" && typeof a.ref === "string")
      .map((a) => a.ref);
    if (ids.length === 0) continue;
    entries.push({ name, identifying: ids });
  }
  ...
  const lines = [
    ...
    ...entries.map((e) =>
      `  [${JSON.stringify(e.name)}, ${JSON.stringify(e.identifying)}],`
    ),
    `]);`,
    ``,
  ];
  writeFileSync(resolve(SRC_DIR, "entity-identifying.ts"), lines.join("\n"), "utf8");
  return entries.length;
}

function emitMetricTripletLookup(metricGroups) {
  // Sidecar lookup the linter rule consumes: metric attribute id → its
  // sibling unit + instrument. The triplet (name, unit, instrument) is
  ...
  for (const m of metricGroups) {
    if (typeof m.metric_name !== "string") continue;
    if (typeof m.unit !== "string" || typeof m.instrument !== "string") continue;
    entries.push({ metric: m.metric_name, unit: m.unit, instrument: m.instrument });
  }
  ...
  const lines = [
    ...
    ...entries.map((e) =>
      `  [${JSON.stringify(e.metric)}, { metric: ${JSON.stringify(e.metric)}, unit: ${JSON.stringify(e.unit)}, instrument: ${JSON.stringify(e.instrument)} }],`
    ),
    `]);`,
    ``,
  ];
  writeFileSync(resolve(SRC_DIR, "metric-triplets.ts"), lines.join("\n"), "utf8");
  return entries.length;
}

function emitEnumKeyedAttrsLookup(byId) {
  // Sidecar lookup: attribute id → the matching `OTel.Enums.<…>` symbol path
  // (only for attribute_group entries whose `type:` is a member-style object).
  ...
  for (const [id, attr] of [...byId.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    const enumSymbol = `OTel.Enums.${pascalCase(domain)}.${pascalCase(domain)}${pascalCase(tail)}`;
    entries.push({ attr: id, enum: enumSymbol });
  }
  const lines = [
    `// <auto-generated/>`,
    `// Source: open-telemetry/semantic-conventions @ ${PINNED_VERSION}`,
    ...
    ...entries.map((e) =>
      `  [${JSON.stringify(e.attr)}, ${JSON.stringify(e.enum)}],`
    ),
    `]);`,
    ``,
  ];
  writeFileSync(resolve(SRC_DIR, "enum-keyed-attrs.ts"), lines.join("\n"), "utf8");
  return entries.length;
}
...
function main() {
  ...
  const metricTripletsCount = emitMetricTripletLookup(partitioned.metrics);
  const enumKeyedCount = emitEnumKeyedAttrsLookup(byId);
  const entityIdentifyingCount = emitEntityIdentifyingLookup(partitioned.entities);
  const knownDomainsCount = emitKnownDomainsLookup(byId);
  ...
  console.log(
    [
      ...
      [
        ...
        ` metric-triplets=${metricTripletsCount}`,
        ` enum-keyed-attrs=${enumKeyedCount}`,
        ` entity-identifying=${entityIdentifyingCount}`,
      ].join(""),
  );
}

// Only run the full regen when invoked as the entry point. Importing this
// module from a test (or any other consumer) yields the named exports above
// without writing any files into lib/.
